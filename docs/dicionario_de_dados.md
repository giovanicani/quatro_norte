...
A extração gera **7 arquivos CSV** que, juntos, permitem calcular o **custo de manutenção interno por quilômetro** das carretas próprias da Trailcon. O modelo é estrela: uma dimensão de carretas (`dim_carretas`) e seis fatos ligados a ela.
...
Todos os 6 arquivos compartilham o mesmo filtro de carretas:
...
Custo interno = linhas de OS com `charge_flag = 'I'` (absorvido pela empresa, não faturado ao cliente). 
...
1. **Custo por OS:** `total_interno_mao_obra + total_interno_pecas` (de `fato_wo`), ou somar as linhas de `fato_wo_labour` + `fato_wo_parts` por `id_os`.
