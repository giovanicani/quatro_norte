...
# Anti-vazamento: regiao_operacao na base e derivada das OS do proprio mes.
# Para modelagem, usar apenas a regiao conhecida ate o mes ANTERIOR (shift por carreta).
model_data = model_data.sort_values(["id_carreta", "ano_mes"])
model_data["regiao_operacao"] = (
    model_data.groupby("id_carreta")["regiao_operacao"].shift(1).fillna("SEM_INFORMACAO")
)
population_summary = (
    ...
    .agg(
        ...
        share_zero_custo_preventivo=("custo_preventivo_total_mes", lambda s: float((s == 0).mean())),
    )
    .reset_index()
)
...
split_summary = pd.DataFrame([
    {"particao": "treino", "linhas": len(train), "inicio": train["ano_mes"].min(), "fim": train["ano_mes"].max(), "share_zero_alvo": float((train[target] == 0).mean())},
    {"particao": "teste", "linhas": len(test), "inicio": test["ano_mes"].min(), "fim": test["ano_mes"].max(), "share_zero_alvo": float((test[target] == 0).mean())},
])
...
if mirror_target in model_data.columns:
    ...
    mirror_rows = [
        {
            "alvo": target,
            "descricao": "alvo principal: custo interno total (mao de obra + pecas; charge_flag=I) por km deflacionado",
